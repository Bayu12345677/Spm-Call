	...
	__import__::app(){
		...
		if (test -f "$__url__"); then
			for __export__ in $(cat "$__url__"); do
				builtin source <(curl -sL --max-time 4 --connect-timeout 3 --parallel --parallel-max 15 "$__export__") || { cat <<< "[*] gagal mengimport pake [$__export__]"; exit 77; }
				imported+=$(echo "$__export__")
			done
		else
			builtin source <(curl -sL --max-time 4 --connect-timeout 3 --parallel --parallel-max 15 "$__url__") || { cat <<< "[*] gagal mengimport pake [$__export__]"; exit 77; }
		fi
	}
...
Std::Main(){
	...
	function async(){ local __command__="$@" eval "$__command__ &"; cat <<< "$!"; }
	alias await="wait"
